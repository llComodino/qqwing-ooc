...
struct sudoku_board_t {
  bool ( *set_puzzle )( int* init_puzzle );
  ...
  int ( *count_solutions )( self_t self );
  int ( *count_solutions_limited )( self_t self );
  ...
  while ( !done ) {
    ...
    if ( action == ACTION_GENERATE ) {
      ...
      if ( read_puzzle_from_stdin( puzzle )) {
        have_puzzle = ss->set_puzzle( puzzle );
        if ( !have_puzzle ) {
          if ( print_puzzle ) {
            ss->print_puzzle( ss );
            printed_something = true;
    ...
    if ( have_puzzle ) {
      ...
      // mess up the stats. )
      if ( count_solutions ) {
        solutions = ss->count_solutions( ss );
      }
