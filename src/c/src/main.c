...
main (int argc, char *argv[])
{
      ...
      if (!strcmp (argv[i], "--puzzle"))
      ...
      else if (!strcmp (argv[i], "--difficulty"))
        {
          if (argc <= i + 1)
            ...
            {
              fprintf (stdout, "%s%s\n",
                       "Difficulty expected to be simple, easy, intermediate, "
                       "expert, or any, not ",
                       argv[i + 1]);
              return 1;
            }
      ...
      else if (!strcmp (argv[i], "--symmetry"))
        {
          if (argc <= i + 1)
            ...
            {
              fprintf (stdout, "%s%s\n",
                       "Symmetry expected to be none, rotate90, rotate180, "
                       "mirror, flip, or random, not ",
                       argv[i + 1]);
              return 1;
            }
          i++;
  ...
  if (print_style == CSV)
    {
      if (print_puzzle)
        fprintf (stdout, "%s", "Puzzle,");
      if (print_solution)
        fprintf (stdout, "%s", "Solution,");
      if (print_history)
        fprintf (stdout, "%s", "Solve History,");
      if (print_instructions)
        fprintf (stdout, "%s", "Solve Instructions,");
      if (count_solutions)
        fprintf (stdout, "%s", "Solution Count,");
      if (timer)
        fprintf (stdout, "%s", "Time ( milliseconds ),");
      if (print_stats)
        fprintf (stdout, "%s",
                 "Givens,Singles,Hidden Singles,Naked Pairs,Hidden "
                 "Pairs,Pointing Pairs/Triples,Box/Line "
                 "Intersections,Guesses,Backtracks,Difficulty");
      fprintf (stdout, "%s\n", "");
    }
  ...
  ss->set_record_history (ss, print_history || print_instructions
                                  || print_stats || difficulty != UNKNOWN);
      ...
      if (have_puzzle)
        {
          ...
          // Solve the puzzle
          if (print_solution || print_history || print_stats
              || print_instructions || difficulty != UNKNOWN)
            {
              ss->solve (ss);
            }
          ...
          if (action == ACTION_GENERATE)
            {
              if (difficulty != UNKNOWN
                  && difficulty != ss->get_difficulty (ss))
                {
                  have_puzzle = false;
                }
              else
                {
                  ...
                  if (number_generated >= number_to_generate)
                    done = true;
                }
            }
        }
      ...
      if (have_puzzle)
        {
          ...
          if (print_history)
            ss->print_solve_history (ss);
          // Print the instructions for solving the puzzle
          if (print_instructions)
            ss->print_solve_instructions (ss);
          ...
          if (timer)
            {
              double t
                  = ((double)(puzzle_done_time - puzzle_start_time)) / 1000.0;
              if (print_style == CSV)
                {
          ...
          if (print_stats)
            {
              ...
              int pointing_pair_triple_count
                  = ss->get_pointing_pair_triple_count (ss);
              ...
              if (print_style == CSV)
                {
                  fprintf (stdout, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%s,",
                           ...
                           guess_count, backtrack_count, difficulty_string);
                }
              else
                {
                  fprintf (stdout, "%s%d\n",
                  ...
                  fprintf (stdout, "%s%d\n",
                           "Number of Pointing Pairs/Triples: ",
                           pointing_pair_triple_count);
                  fprintf (stdout, "%s%d\n",
                           "Number of Box/Line Intersections: ",
                           box_reduction_count);
                  fprintf (stdout, "%s%d\n",
  ...
  if (timer)
    {
      double t = ((double)(application_done_time - application_start_time))
                 / 1000000.0;
      fprintf (stdout, "%d%s%s%s%s%s%.3f%s\n", puzzle_count, " puzzle",
               ((puzzle_count == 1) ? "" : "s"), " ",
               (action == ACTION_GENERATE ? "generated" : "solved"), " in ", t,
               " seconds.");
    }
...
print_help (void)
{
  ...
  fprintf (stdout, "%s\n",
           "  --difficulty <diff>  Generate only simple, easy, intermediate, "
           "expert, or any");
  fprintf (stdout, "%s\n",
           "  --symmetry <sym>     Symmetry: none, rotate90, rotate180, "
           "mirror, flip, or random");
  fprintf (
      stdout, "%s\n",
      "  --puzzle             Print the puzzle ( default when generating )");
  fprintf (stdout, "%s\n",
           "  --nopuzzle           Do not print the puzzle ( default when "
           "solving )");
  fprintf (
      stdout, "%s\n",
      "  --solution           Print the solution ( default when solving )");
  fprintf (stdout, "%s\n",
           "  --nosolution         Do not print the solution ( default when "
           "generating )");
  fprintf (stdout, "%s\n",
           "  --stats              Print statistics about moves used to solve "
           "the puzzle");
  fprintf (stdout, "%s\n",
  ...
  fprintf (stdout, "%s\n",
           "  --notimer            Do not print solve or generation times ( "
           "default )");
#endif
  fprintf (stdout, "%s\n",
           "  --count-solutions    Count the number of solutions to puzzles");
  fprintf (stdout, "%s\n",
           "  --nocount-solutions  Do not count the number of solutions ( "
           "default )");
  fprintf (stdout, "%s\n",
           "  --history            Print trial and error used when solving");
  fprintf (stdout, "%s\n",
           "  --nohistory          Do not print trial and error to solve ( "
           "default )");
  fprintf (stdout, "%s\n",
           "  --instructions       Print the steps ( at least 81 ) needed to "
           "solve the puzzle");
  fprintf (stdout, "%s\n",
  ...
  fprintf (stdout, "%s\n",
           "  --nolog-history      Do not print trial and error  to solve as "
           "it happens");
  fprintf (
      stdout, "%s\n",
  ...
  fprintf (stdout, "%s\n",
           "  --readable           Print puzzles in human readable form ( "
           "default )");
  fprintf (stdout, "%s\n",
