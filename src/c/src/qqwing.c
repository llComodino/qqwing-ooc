...
bool solve( _self_t self );
bool _solve( _self_t, int round );
int count_solutions( _self_t self, bool limited );
int _count_solutions( _self_t self, int round, bool limited );
...
bool remove_possibilities_in_one_from_two( _self_t self, int pos_1, int pos_2, int round );
...
struct sudoku_board_t {
  ...
  bool ( *reset)(_self_t );
  bool ( *single_solve_move)(_self_t, int );
  bool ( *only_possibility_for_cell)(_self_t, int );
  bool ( *only_value_in_row)(_self_t, int );
  bool ( *only_value_in_column)(_self_t, int );
  bool ( *only_value_in_section)(_self_t, int );
  bool ( *solve)(_self_t );
  int ( *count_solutions)(_self_t, bool );
  bool ( *guess)(_self_t, int, int );
  bool ( *is_impossible)(_self_t );
  void ( *rollback_round)(_self_t, int );
  bool ( *pointing_row_reduction)(_self_t, int );
  bool ( *row_box_reduction)(_self_t, int );
  bool ( *col_box_reduction)(_self_t, int );
  bool ( *pointing_column_reduction)(_self_t, int );
  bool ( *hidden_pair_in_row)(_self_t, int );
  bool ( *hidden_pair_in_column)(_self_t, int );
  bool ( *hidden_pair_in_section)(_self_t, int );
  void ( *mark)(_self_t, int, int, int );
  int ( *find_position_with_fewest_possibilities)(_self_t );
  ...
  bool ( *remove_possibilities_in_one_from_two)(_self_t, int, int, int );
};
...
{
  struct sudoku_board_t* board = ( sudoku_board_t*)malloc(sizeof( sudoku_board_t ) );
  ...
  board->random_possibility_array = ( int* )malloc(sizeof( int ) * ROW_COL_SEC_SIZE );
  ...
  board->mark = mark;
  board->find_position_with_fewest_possibilities = find_position_with_fewest_possibilities;
  ...
  board->remove_possibilities_in_one_from_two = remove_possibilities_in_one_from_two;
 ...
 */
int
get_given_count( struct sudoku_board_t *const restrict self )
{
  int count = 0;
  for ( int i = 0; i < BOARD_SIZE; i++ ) {
    if ( self->puzzle[i] != 0 ) count++;
  }
  return count;
}
 ...
 */
bool
set_puzzle( struct sudoku_board_t *const restrict self, int* init_puzzle )
{
  for ( int i = 0; i < BOARD_SIZE; i++ ) {
 ...
 */
const int*
get_puzzle( struct sudoku_board_t *const restrict self )
{
  return self->puzzle;
}
 ...
 */
const int*
get_solution( struct sudoku_board_t *const restrict self )
{
 ...
 */
bool
reset( struct sudoku_board_t *const restrict self )
{
  for ( int i = 0; i < BOARD_SIZE; i++ ) {
    self->solution[i] = 0;
  }
  ...
  for ( unsigned int i = 0; i < self->solve_history->size; i++ ) {
    free( self->solve_history + i );
  }
  ...
  for ( int position = 0; position < BOARD_SIZE; position++ ) {
    if ( self->puzzle[position] > 0 ) {
      ...
      if ( self->possibilities[val_pos] != 0 ) return false;
      mark( self, position, round, value );
      if ( self->log_history || self->record_history ) add_history_item(
                                                       self,
                                                       new_val_pos_log(
                                                         round,
                                                         GIVEN,
 ...
 */
difficulty_t
get_difficulty( struct sudoku_board_t *const restrict self )
{
  if ( get_guess_count( self ) > 0 ) return EXPERT;
  if ( get_box_line_reduction_count( self ) > 0 ) return INTERMEDIATE;
  if ( get_pointing_pair_triple_count( self ) > 0 ) return INTERMEDIATE;
  if ( get_hidden_pair_count( self ) > 0 ) return INTERMEDIATE;
  if ( get_naked_pair_count( self ) > 0 ) return INTERMEDIATE;
  if ( get_hidden_single_count( self ) > 0 ) return EASY;
  if ( get_single_count( self ) > 0 ) return SIMPLE;
  return UNKNOWN;
}
 ...
 */
char*
get_difficulty_as_string( struct sudoku_board_t *const restrict self )
{
 ...
 */
int
get_single_count( struct sudoku_board_t *const restrict self )
{
 ...
 */
int
get_hidden_single_count( struct sudoku_board_t *const restrict self )
{
 ...
 */
int
get_naked_pair_count( struct sudoku_board_t *const restrict self )
{
 ...
 */
int
get_hidden_pair_count( struct sudoku_board_t *const restrict self )
{
 ...
 */
int
get_pointing_pair_triple_count( struct sudoku_board_t *const restrict self )
{
  return get_log_count( self->solve_instructions, POINTING_PAIR_TRIPLE_ROW )+
    get_log_count( self->solve_instructions, POINTING_PAIR_TRIPLE_COLUMN );
}
 ...
 */
int
get_box_line_reduction_count( struct sudoku_board_t *const restrict self )
{
 ...
 */
int
get_guess_count( struct sudoku_board_t *const restrict self )
{
 ...
 */
int
get_backtrack_count( struct sudoku_board_t *const restrict self )
{
  return get_log_count( self->solve_history, ROLLBACK );
}

void
shuffle_random_arrays( struct sudoku_board_t *const restrict self )
{
...
void
clear_puzzle( struct sudoku_board_t *const restrict self )
{
...
bool
generate_puzzle_symmetry( _self_t self, symmetry_t symmetry )
{
  if ( symmetry == SYMMETRY_RANDOM) symmetry = get_random_symmetry();

  // Don't record history while generating.
  bool rec_hist = self->record_history;
  set_record_history( self, false );
  bool log_hist = self->log_history;
  set_log_history( self, false );
    ...
    if ( self->puzzle[position] > 0 ) {
      ...
      switch ( symmetry ) {
        case SYMMETRY_ROTATE90:
          pos_sym_2 = row_column_to_cell( ROW_COL_SEC_SIZE - 1 - cell_to_column( position ), cell_to_row( position ) );
          pos_sym_3 = row_column_to_cell( cell_to_column( position ), ROW_COL_SEC_SIZE - 1 - cell_to_row( position ) );
        case SYMMETRY_ROTATE180:
          pos_sym_1 = row_column_to_cell( ROW_COL_SEC_SIZE - 1 - cell_to_row( position ), ROW_COL_SEC_SIZE - 1 - cell_to_column( position ) );
        break;
        case SYMMETRY_MIRROR:
          pos_sym_1 = row_column_to_cell( cell_to_row( position ), ROW_COL_SEC_SIZE - 1 - cell_to_column( position ) );
        break;
        case SYMMETRY_FLIP:
          pos_sym_1 = row_column_to_cell( ROW_COL_SEC_SIZE - 1 - cell_to_row( position ), cell_to_column( position ) );
        break;
      ...
      reset( self );
      if ( _count_solutions( self, 2, true ) > 1 ) {
        // Put it back in, it is needed
        self->puzzle[position] = saved_value;
        if ( pos_sym_1 >= 0 && saved_sym_1 != 0 ) self->puzzle[pos_sym_1] = saved_sym_1;
        if ( pos_sym_2 >= 0 && saved_sym_2 != 0 ) self->puzzle[pos_sym_2] = saved_sym_2;
        if ( pos_sym_3 >= 0 && saved_sym_3 != 0 ) self->puzzle[pos_sym_3] = saved_sym_3;
      }
    }
  }
  ...
  set_record_history( self, rec_hist );
  set_log_history( self, log_hist );
...
void
set_print_style( _self_t self, print_style_t ps )
{
  self->print_style = ps;
}

void
set_record_history( _self_t self, bool rec_hist )
{
  self->record_history = rec_hist;
}

void
set_log_history( _self_t self, bool log_hist )
{
...
void
print_solve_instructions( _self_t self )
{
  if ( is_solved( self ) ) {
...
void
print_solve_history( _self_t self )
{
...
solve( _self_t self )
{
  ...
  return _solve( self, 2 );
}

bool
_solve( _self_t self, int round )
{
  self->last_solve_round = round;

  while ( single_solve_move(self, round) ) {
    if ( is_solved( self ) ) return true;
    if ( is_impossible( self ) ) return false;
  }

  int next_guess_round = round + 1;
  int next_round = round + 2;
  for ( int guess_number = 0; guess(self, next_guess_round, guess_number); guess_number++ ) {
    if ( is_impossible( self ) || !_solve(self, next_round) ) {
...
bool
has_unique_solution( _self_t self )
{
  return count_solutions( self, true ) == 1;
}

int
count_solutions( _self_t self, bool limit_to_two )
{
  // Don't record history while generating.
  bool rec_hist = self->record_history;
  set_record_history( self, false );
  ...
  reset( self );
  int solution_count = _count_solutions( self, 2, limit_to_two );
...
int
_count_solutions( _self_t self, int round, bool limit_to_two )
{
  while ( single_solve_move(self, round) ) {
    if ( is_solved( self ) ) {
  ...
  for ( int guess_number = 0; guess(self, next_round, guess_number); guess_number++ ) {
    solutions += _count_solutions( self, next_round, limit_to_two );
...
{
  if ( self->log_history || self->record_history) add_history_item(self, new_std_log(round, ROLLBACK) );
  for ( int i = 0; i < BOARD_SIZE; i++ ) {
  ...
  while( s_ins->size > 0 && s_ins->tail->item->get_round(s_ins->tail->item) == round ) {
    self->solve_instructions->pop( self->solve_instructions );
  }
}

bool
is_solved( _self_t self )
{
  for ( int i = 0; i < BOARD_SIZE; i++ ) {
  ...
  for ( int position = 0; position < BOARD_SIZE; position++ ) {
    if ( self->solution[position] == 0 ) {
      ...
      for ( int val_idx = 0; val_idx < ROW_COL_SEC_SIZE; val_idx++ ) {
        int val_pos = get_possibility_index( val_idx, position );
        if ( self->possibilities[val_pos] == 0 ) count++;
      }
      if ( count == 0 ) {
        return true;
      }
...
{
  if ( only_possibility_for_cell(self, round) ) return true;
  if ( only_value_in_section(self, round) ) return true;
  if ( only_value_in_row(self, round) ) return true;
  if ( only_value_in_column(self, round) ) return true;
  if ( handle_naked_pairs(self, round) ) return true;
  if ( pointing_row_reduction(self, round) ) return true;
  if ( pointing_column_reduction(self, round) ) return true;
  if ( row_box_reduction(self, round) ) return true;
  if ( col_box_reduction(self, round) ) return true;
  if ( hidden_pair_in_row(self, round) ) return true;
  if ( hidden_pair_in_column(self, round) ) return true;
  if ( hidden_pair_in_section(self, round) ) return true;
  return false;
}
  ...
  for ( int val_idx = 0; val_idx < ROW_COL_SEC_SIZE; val_idx++ ) {
    for ( int col = 0; col < ROW_COL_SEC_SIZE; col++ ) {
      ...
      if ( in_one_box && col_box != -1 ) {
        ...
        int sec_start = cell_to_section_start_cell( row_column_to_cell(row, col) );
  ...
  for ( int val_idx = 0; val_idx < ROW_COL_SEC_SIZE; val_idx++ ) {
    for ( int row = 0; row < ROW_COL_SEC_SIZE; row++ ) {
      ...
      if ( in_one_box && row_box != -1 ) {
        ...
        int sec_start = cell_to_section_start_cell( row_column_to_cell(row, column) );
        int sec_start_row = cell_to_row( sec_start );
        int sec_start_col = cell_to_column( sec_start );
        for ( int i = 0; i < GRID_SIZE; i++ ) {
          for ( int j = 0; j < GRID_SIZE; j++ ) {
        ...
        if ( done_something ) {
          if ( self->log_history || self->record_history) add_history_item(self, new_val_pos_log(round, ROW_BOX, val_idx + 1, row_start) );
          return true;
        }
      }
    }
  ...
  for ( int val_idx = 0; val_idx < ROW_COL_SEC_SIZE; val_idx++ ) {
    for ( int section = 0; section < ROW_COL_SEC_SIZE; section++ ) {
      ...
      if ( in_one_row && box_row != -1 ) {
        ...
        if ( done_something ) {
          if ( self->log_history || self->record_history) add_history_item(self, new_val_pos_log(round, POINTING_PAIR_TRIPLE_ROW, val_idx + 1, row_start) );
          return true;
        }
      }
    }
  ...
  for ( int val_idx = 0; val_idx < ROW_COL_SEC_SIZE; val_idx++ ) {
    for ( int section = 0; section < ROW_COL_SEC_SIZE; section++ ) {
      ...
      if ( in_one_col && box_col != -1 ) {
        ...
        if ( done_something ) {
          if ( self->log_history || self->record_history) add_history_item(self, new_val_pos_log(round, POINTING_PAIR_TRIPLE_COLUMN, val_idx + 1, col_start) );
          return true;
        }
      }
    }
...
bool
remove_possibilities_in_one_from_two( _self_t self, int position1, int position2, int round )
{
  bool done_something = false;
  for ( int val_idx = 0; val_idx < ROW_COL_SEC_SIZE; val_idx++ ) {
    int val_pos1 = get_possibility_index( val_idx, position1 );
...
hidden_pair_in_column( _self_t self, int round )
{
  for ( int column = 0; column < ROW_COL_SEC_SIZE; column++ ) {
    for ( int val_idx = 0; val_idx < ROW_COL_SEC_SIZE; val_idx++ ) {
      ...
      if ( val_count == 2 ) {
        for ( int val_idx2 = val_idx + 1; val_idx2 < ROW_COL_SEC_SIZE; val_idx2++ ) {
          int r3 = -1;
          int r4 = -1;
          int val_count2 = 0;
          for ( int row = 0; row < ROW_COL_SEC_SIZE; row++ ) {
          ...
          if ( val_count2 == 2 && r1 == r3 && r2 == r4 ) {
            bool done_something = false;
            for ( int val_idx3 = 0; val_idx3 < ROW_COL_SEC_SIZE; val_idx3++ ) {
              if ( val_idx3 != val_idx && val_idx3 != val_idx2 ) {
                ...
                int val_pos1  = get_possibility_index( val_idx3, position1 );
                int val_pos2  = get_possibility_index( val_idx3, position2 );
    ...
    if ( possibilities == 2 ) {
      ...
      for ( int position2 = position; position2 < BOARD_SIZE; position2++ ) {
        if ( position != position2 ) {
          ...
          if ( possibilities2 == 2 && are_possibilities_same(self, position, position2) ) {
            if ( row == cell_to_row(position2) ) {
              bool done_something = false;
              for ( int column2 = 0; column2 < ROW_COL_SEC_SIZE; column2++ ) {
                int position3 = row_column_to_cell( row, column2 );
                if ( position3 != position && position3 != position2 && remove_possibilities_in_one_from_two(self, position, position3, round) ) {
                  done_something = true;
                }
              }
              if ( done_something ) {
                if ( self->log_history || self->record_history) add_history_item(self, new_val_pos_log(round, NAKED_PAIR_ROW, 0, position) );
                return true;
              }
            }
            if ( column == cell_to_column(position2) ) {
              ...
              for ( int row2 = 0; row2 < ROW_COL_SEC_SIZE; row2++ ) {
                int position3 = row_column_to_cell( row2, column );
                if ( position3 != position && position3 != position2 && remove_possibilities_in_one_from_two(self, position, position3, round) ) {
                  done_something = true;
                }
              }
              if ( done_something ) {
                if ( self->log_history || self->record_history) add_history_item(self, new_val_pos_log(round, NAKED_PAIR_COLUMN, 0, position) );
                return true;
              }
            }
            if ( section == cell_to_section_start_cell(position2) ) {
              ...
              for ( int i = 0; i < GRID_SIZE; i++ ) {
                for ( int j = 0; j < GRID_SIZE; j++ ) {
                  int position3 = sec_start + i+( ROW_COL_SEC_SIZE * j );
                  if ( position3 != position && position3 != position2 && remove_possibilities_in_one_from_two(self, position, position3, round) ) {
                    done_something = true;
                  }
                }
              ...
              if ( done_something ) {
                if ( self->log_history || self->record_history) add_history_item(self, new_val_pos_log(round, NAKED_PAIR_SECTION, 0, position) );
                return true;
              }
            }
          }
...
only_value_in_row( _self_t self, int round )
{
  for ( int row = 0; row < ROW_COL_SEC_SIZE; row++ ) {
    for ( int val_idx = 0; val_idx < ROW_COL_SEC_SIZE; val_idx++ ) {
      ...
      if ( count == 1 ) {
        int value = val_idx + 1;
        if ( self->log_history || self->record_history) add_history_item(self, new_val_pos_log(round, HIDDEN_SINGLE_ROW, value, last_position) );
...
void
print_puzzle( _self_t self )
{
  print( self, self->puzzle );
}

void
print_solution( _self_t self )
{
  print( self, self->solution );
}

log_item_t* new_std_log( int r, log_type_t t ) {
...
log_item_t* new_val_pos_log( int r, log_type_t t, int v, int p ) {
  log_item_t* item = ( log_item_t*)malloc(sizeof( log_item_t ) );
...
_print( const log_item_t *const restrict self )
{
  ...
  switch( self->get_type( self ) ) {
    ...
    case POINTING_PAIR_TRIPLE_ROW:
      fprintf( stdout, "%s", "Remove possibilities for row because all values are in one section" );
    break;
    case POINTING_PAIR_TRIPLE_COLUMN:
      fprintf( stdout, "%s", "Remove possibilities for column because all values are in one section" );
    break;
    case ROW_BOX:
      fprintf( stdout, "%s", "Remove possibilities for section because all values are in one row" );
    break;
    case COLUMN_BOX:
      fprintf( stdout, "%s", "Remove possibilities for section because all values are in one column" );
    break;
    case HIDDEN_PAIR_ROW:
      fprintf( stdout, "%s", "Remove possibilities from hidden pair in row" );
    break;
    case HIDDEN_PAIR_COLUMN:
      fprintf( stdout, "%s", "Remove possibilities from hidden pair in column" );
    break;
    case HIDDEN_PAIR_SECTION:
      fprintf( stdout, "%s", "Remove possibilities from hidden pair in section" );
    break;
    default:
  ...
  if ( self->value > 0 || self->position > -1 ) {
    ...
    bool printed = false;
    if ( self->position > -1 ) {
      if ( printed) fprintf(stdout, "%s", " - " );
      fprintf( stdout, "%s%d%s%d", "Row: ", cell_to_row(self->position) + 1, " - Column: ", cell_to_column(self->position) + 1 );
      printed = true;
    }
    if ( self->value > 0 ) {
      if ( printed) fprintf(stdout, "%s", " - " );
...
get_log_count( const log_list_t *const restrict list, log_type_t type )
{
  ...
  for ( unsigned int i = 0; i < list->size; i++ ) {
    if( list->at(list, i)->get_type(list->at(list, i)) == type ) count++;
  }
  return count;
}
...
{
  switch ( rand() % 4 ) {
    case 0: return SYMMETRY_ROTATE90;
    case 1: return SYMMETRY_ROTATE180;
    case 2: return SYMMETRY_MIRROR;
    case 3: return SYMMETRY_FLIP;
  }
 ...
 */
static inline
int cell_to_section_start_cell( int cell )
{
 ...
 */
static inline
int column_to_first_cell( int column )
{
  return column;
}
 ...
 */
static inline
int section_to_first_cell( int section )
{
 ...
 */
static inline
int get_possibility_index( int value_index, int cell )
{
 ...
 */
static inline
int row_column_to_cell( int row, int column )
{
 ...
 */
static inline
int section_to_cell( int section, int offset )
{
